DROP TABLE IF EXISTS `borhandw`.`dwh_dim_entry_media_source`;

CREATE TABLE `borhandw`.`dwh_dim_entry_media_source` (
...
CREATE TRIGGER `borhandw`.`dwh_dim_entry_media_source_setcreationtime_oninsert` BEFORE INSERT
    ON `borhandw`.`dwh_dim_entry_media_source`
    FOR EACH ROW 
	SET new.dwh_creation_date = NOW();
	
INSERT INTO borhandw.dwh_dim_entry_media_source (entry_media_source_id, entry_media_source_name, entry_media_source_category, dwh_creation_date, dwh_update_date, ri_ind)
