...
ALTER TABLE borhandw.dwh_fact_events ADD COLUMN entry_partner_id INT after entry_media_type_id;

# update index on borhandw.dwh_fact_events
ALTER TABLE borhandw.dwh_fact_events ADD  KEY event_partner_id_event_type_id_time (`entry_partner_id`,`event_type_id`,`event_time` );

# add field on borhandw_ds.DS_Events
ALTER TABLE borhandw_ds.DS_Events ADD COLUMN entry_partner_id INT after entry_media_type_id;
...
BEGIN
	SET @s = CONCAT('insert into borhandw.dwh_fact_events
			 ...
			 process_duration, control_id, seek, new_point, domain_id, referrer , entry_media_type_id , entry_partner_id from borhandw_ds.ds_events 
