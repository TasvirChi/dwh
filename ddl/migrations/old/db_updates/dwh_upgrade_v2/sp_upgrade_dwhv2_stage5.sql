...
CREATE PROCEDURE  `borhandw`.`monthly_partner_billing_report`( month_id int )
BEGIN
...
SET @current_month = month_id;
	SELECT 
		...
		borhandw.get_secondary_partners_as_string(calculated_stats.partner_id) "partners in group",
	...
	(
		SELECT 
			...
			borhandw.get_primary_partner(aggr_single_partner.partner_id) "partner_id",
		...
		(
			SELECT
				borhandw.calc_month_id(aggr_partner.date_id) month_id, 
				...
				borhandw.calc_partner_storage_data_last_month(@current_month , aggr_partner.partner_id) billing_storage_mb , 
				SUM(IF(borhandw.calc_month_id(aggr_partner.date_id)=@current_month,aggr_partner.count_bandwidth,NULL)) sum_bandwith_for_month_aggr_kb  ,
				SUM(IF(borhandw.calc_month_id(aggr_partner.date_id)=@current_month,aggr_partner.count_storage,NULL)) sum_count_storage_for_month_aggr_mb  ,
				SUM(IF(borhandw.calc_month_id(aggr_partner.date_id)=@current_month,aggr_partner.count_streaming,NULL)) sum_streaming_for_month_aggr_kb  
			FROM 
				borhandw.dwh_aggr_partner aggr_partner, 
				borhandw.dwh_dim_partners inner_dim_partner USE INDEX (partner_package_indx)
			WHERE 
				borhandw.calc_month_id(aggr_partner.date_id)<=@current_month 
		...
		GROUP BY 
			borhandw.get_primary_partner(aggr_single_partner.partner_id)
		ORDER BY 
			borhandw.get_primary_partner(aggr_single_partner.partner_id) ASC
	) calculated_stats ,
			borhandw.dwh_dim_partners dim_partner 
	WHERE
		calculated_stats.partner_id=dim_partner.partner_id
		AND dim_partner.partner_package>1;
END $$

DROP PROCEDURE `borhandw`.`monthly_non_paying_billing_report` $$

CREATE PROCEDURE  `borhandw`.`monthly_non_paying_billing_report`( month_id INT )
BEGIN
SET @current_month = month_id;
	SELECT 
		calculated_stats.month_id,
	...
	(
		...
		(
			...
			(
				...
				FROM
					borhandw.dwh_aggr_partner inner_aggr_partner ,
					borhandw.dwh_dim_partners inner_dim_partner
				WHERE 
		...
		ORDER BY
			...
			borhandw.dwh_dim_partners dim_partner ,
			borhandw.dwh_dim_partner_type dim_partner_type
	WHERE
		calculated_stats.partner_id=dim_partner.partner_id
