...
BEGIN
	DELETE FROM borhandw.dwh_hourly_partner_usage WHERE date_id = date(date_val)*1 and IFNULL(count_bandwidth_kb,0) = 0 and (ifnull(count_storage_mb,0) > 0 or ifnull(billable_storage_mb,0) > 0);
    UPDATE borhandw.dwh_hourly_partner_usage SET count_storage_mb = null, billable_storage_mb=null WHERE date_id = date(date_val)*1 and IFNULL(count_bandwidth_kb,0) > 0;
	...
	INSERT INTO 	borhandw.dwh_hourly_partner_usage (partner_id, date_id, hour_id, bandwidth_source_id, count_storage_mb)
	...
	INSERT INTO 	borhandw.dwh_hourly_partner_usage (partner_id, date_id, hour_id, bandwidth_source_id, billable_storage_mb)
