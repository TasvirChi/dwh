...
BEGIN
	...
	SELECT u.kuser_id , SUM(s.entry_additional_size_kb) storage_kb FROM 
		(SELECT DISTINCT entry_id FROM borhandw.dwh_fact_entries_sizes s 
				INNER JOIN (SELECT DISTINCT aggr_day_int FROM borhandw.aggr_managment WHERE is_calculated = 0 AND aggr_day < max_date AND aggr_name = 'storage_usage_kuser_sync') aggr_managment
