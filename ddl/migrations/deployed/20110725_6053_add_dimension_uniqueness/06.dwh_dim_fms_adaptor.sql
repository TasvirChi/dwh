/*
SELECT d.* FROM borhandw.dwh_dim_fms_adaptor d, 
		(SELECT adaptor, COUNT(*) FROM borhandw.dwh_dim_fms_adaptor
			GROUP BY adaptor
			HAVING COUNT(*) > 1) dup
WHERE d.adaptor = dup.adaptor;

DELETE FROM borhandw.dwh_dim_fms_adaptor
WHERE adaptor_id = 4;
...
ALTER TABLE borhandw.dwh_dim_fms_adaptor ADD UNIQUE KEY (adaptor);
