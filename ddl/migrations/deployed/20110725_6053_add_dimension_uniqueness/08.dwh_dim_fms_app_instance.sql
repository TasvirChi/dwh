/*SELECT d.* FROM borhandw.dwh_dim_fms_app d, 
		(SELECT fms_app_name, COUNT(*) FROM borhandw.dwh_dim_fms_app
			GROUP BY fms_app_name
			HAVING COUNT(*) > 1) dup
WHERE d.fms_app_name = dup.fms_app_name;

DELETE FROM borhandw.dwh_dim_fms_app
WHERE fms_app_id IN (10,13,23,25,32,35,36,38,39,42,43,46,47);

UPDATE  borhandw.dwh_fact_fms_session_events
...
ALTER TABLE borhandw.dwh_dim_fms_app_instance CHANGE app_instance app_instance VARCHAR(333);
ALTER TABLE borhandw.dwh_dim_fms_app_instance ADD UNIQUE KEY (app_instance);
