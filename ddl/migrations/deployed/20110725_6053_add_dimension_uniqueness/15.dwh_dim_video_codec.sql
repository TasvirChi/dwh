/*
SELECT d.* FROM borhandw.dwh_dim_video_codec d, 
		(SELECT video_codec, COUNT(*) FROM borhandw.dwh_dim_video_codec
			GROUP BY video_codec
			HAVING COUNT(*) > 1) dup
WHERE d.video_codec = dup.video_codec;

DELETE FROM borhandw.dwh_dim_video_codec WHERE video_codec_id IN (15,18, 34,154,177);
...
ALTER TABLE borhandw.dwh_dim_video_codec CHANGE video_codec video_codec varchar(333);
ALTER TABLE borhandw.dwh_dim_video_codec ADD UNIQUE KEY (video_codec);
