...
BEGIN
	...
	IF (p_date_val >= v_ignore) THEN -- not so old, we don't have any data
		...
		DELETE FROM borhandw.dwh_hourly_partner_usage WHERE date_id = date(p_date_val)*1 and IFNULL(count_bandwidth_kb,0) > 0 and ifnull(count_storage_mb,0) = 0 and ifnull(aggr_storage_mb,0) = 0;
		UPDATE borhandw.dwh_hourly_partner_usage SET count_bandwidth_kb = null WHERE date_id = date(p_date_val)*1 and (ifnull(count_storage_mb,0) > 0 or ifnull(aggr_storage_mb,0) > 0);
		...
		WHERE table_name = 'dwh_fact_bandwidth_usage';
                ...
                SET @s = CONCAT('INSERT INTO borhandw.dwh_hourly_partner_usage (partner_id, date_id, hour_id, bandwidth_source_id, count_bandwidth_kb)
