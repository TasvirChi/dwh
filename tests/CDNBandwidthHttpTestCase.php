...
abstract class CDNBandwidthHttpTestCase extends CycleProcessTestCase
{
	...
	public function testProcess()
	{
		...
		foreach($files as $fileID)
		{
			...
			// compare rows in ds_bandwidth_usage to rows in file
                        ...
                        $this->assertEquals(DWHInspector::countRows('borhandw_ds.ds_bandwidth_usage',$fileID, 'and bandwidth_source_id = '. $this->getBandwidthSourceID()),$this->countRows($filename, array($this, 'validBWLine')));

                        // compare bandwidth_bytes in ds_bandwidth_usage to bandwidth bytes consumed in file
                        $this->assertEquals(DWHInspector::sumRows('borhandw_ds.ds_bandwidth_usage',$fileID,"bandwidth_bytes"),$this->sumBytes($filename, array($this, 'validBWLine'), $this->getBWRegex()));
			...
			$this->assertEquals($this->countInvalidLines($filename, array($this, 'validBWLine'), array($this, 'ignoredInvalidBWLine')), DWHInspector::countRows('borhandw_ds.invalid_ds_lines',$fileID));
		}
	}
	...
	public function testAggregation($sourceId)
	{
		...
		$this->compareAggregation(array(new ComparedTable('partner_id', 'borhandw.dwh_fact_bandwidth_usage', '(bandwidth_bytes/1024)')), 
					  array(new ComparedTable('partner_id', 'borhandw.dwh_hourly_partner_usage', 'if(bandwidth_source_id='.$sourceId.', ifnull(count_bandwidth_kb, 0),0)')), 1);
		...
		$this->compareAggregation(array(new ComparedTable('bandwidth_source_id', 'borhandw.dwh_fact_bandwidth_usage', '(bandwidth_bytes/1024)')),
                                          array(new ComparedTable('bandwidth_source_id', 'borhandw.dwh_hourly_partner_usage', 'if(bandwidth_source_id='.$sourceId.', ifnull(count_bandwidth_kb, 0),0)')), 1);
