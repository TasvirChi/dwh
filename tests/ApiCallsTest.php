...
class ApiCallsTest 
{
	...
	public function testProcess()
	{
		...
		foreach($files as $fileID)
		{
			...
			// compare rows in full api_calls to the ones in the file to rows in file
                        $this->assertEquals(DWHInspector::countRows('borhandw_ds.ds_api_calls',$fileID),count($this->getFileApiFullCalls($filename)));
			// compare rows in partial api_calls to the ones in the file to rows in file
                        $this->assertEquals(DWHInspector::countRows('borhandw_ds.ds_incomplete_api_calls',$fileID),count($this->getFileApiIncompleteCalls($filename)));
			...
			$collection = $this->getFullCallsPerEntity($filename, "ACTION", "unknown");
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_api_calls',$fileID,'action_name','borhandw.dwh_dim_api_actions','action_id'), count($collection));
	                foreach($collection as $objectID=>$val)
	                {
	                        $res = DWHInspector::countRows('borhandw_ds.ds_api_calls',$fileID," and  action_name = '$objectID'", 'borhandw.dwh_dim_api_actions','action_id');
			...
			$collection = $this->getFullCallsPerEntity($filename, "SERVICE", "unknown");
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_api_calls',$fileID,'service_name','borhandw.dwh_dim_api_actions','action_id'), count($collection));
			foreach($collection as $objectID=>$val)
                        {
                                $res = DWHInspector::countRows('borhandw_ds.ds_api_calls',$fileID," and  service_name = '$objectID'", 'borhandw.dwh_dim_api_actions','action_id');
			...
			$collection = $this->getFullCallsPerEntity($filename, "PARTNER_ID");
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_api_calls',$fileID,'partner_id'), count($collection));
                        foreach($collection as $objectID=>$val)
                        {
                                $res = DWHInspector::countRows('borhandw_ds.ds_api_calls',$fileID," and  partner_id = '$objectID'");
			...
			{
				$res = DWHInspector::sumRows('borhandw_ds.ds_api_calls', $fileID, 'duration_msecs', " and  partner_id = '$partnerID'");
				$maxDiffInPercent = 1;
			...
			// compare number of distinct tags and number of rows per tags
                        ...
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_api_calls',$fileID,'client_tag_name', 'borhandw.dwh_dim_client_tags', 'client_tag_id'),count($collection));
			foreach($collection as $objectID=>$val)
                        {
                                $res = DWHInspector::countRows('borhandw_ds.ds_api_calls',$fileID," and  client_tag_name = '$objectID'", 'borhandw.dwh_dim_client_tags', 'client_tag_id');
			...
			// compare number of distinct is_admins and number of rows per is_admin
                        ...
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_api_calls',$fileID,'is_admin'),count($collection));
			foreach($collection as $objectID=>$val)
                        {
                                $res = DWHInspector::countRows('borhandw_ds.ds_api_calls',$fileID," and  is_admin = '$objectID'");
			...
			// compare number of distinct partners and number of rows per partner
                        ...
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_errors',$fileID,'partner_id'), count($collection));
                        foreach($collection as $objectID=>$val)
                        {
                                $res = DWHInspector::countRows('borhandw_ds.ds_errors',$fileID," and partner_id = '$objectID'");
                                $this->assertEquals($res, $val, "Expected(db): $res, Actual(file): $val partner_id: $objectID");
                        }


			$collection = $this->getFullCallsPerEntity($filename, "ERROR_CODE", '', true);
                        $this->assertEquals(DWHInspector::countDistinct('borhandw_ds.ds_errors',$fileID,'error_code_name', 'borhandw.dwh_dim_error_codes', 'error_code_id'), count($collection));
                        foreach($collection as $objectID=>$val)
                        {
                                $res = DWHInspector::countRows('borhandw_ds.ds_errors',$fileID," and error_code_name = '$objectID'", 'borhandw.dwh_dim_error_codes', 'error_code_id');
	...
	public function testTransfer()
	{
		...
		{
			$this->assertEquals(DWHInspector::countRows('borhandw.dwh_fact_api_calls', '%', "and CONCAT(session_id,'_',request_index) = '". $call->getID()."'"), 1, "APICall ID ". $call->getID());
			$this->assertEquals(DWHInspector::countRows('borhandw.dwh_fact_incomplete_api_calls', '%', "and CONCAT(session_id,'_',request_index) = '". $call->getID()."'"), 0, "APICall ID ". $call->getID());
			$this->assertEquals(DWHInspector::countRows('borhandw.dwh_fact_errors', '%', "and error_object_id = '". $call->getID()."'"), 1, "APICall ID ". $call->getID());
		}
	}
	...
	public function testAggregation()
	{
		...
		$this->compareAggregation(array(new ComparedTable('partner_id', 'borhandw.dwh_fact_api_calls', 'success')),
                                          array(new ComparedTable('partner_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_success, 0)')));
		$this->compareAggregation(array(new ComparedTable('partner_id', 'borhandw.dwh_fact_api_calls', '*')),
                                          array(new ComparedTable('partner_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_calls, 0)')));
		$this->compareAggregation(array(new ComparedTable('partner_id', 'borhandw.dwh_fact_api_calls', 'duration_msecs')),
                                          array(new ComparedTable('partner_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(sum_duration_msecs, 0)')));
		$this->compareAggregation(array(new ComparedTable('partner_id', 'borhandw.dwh_fact_api_calls', 'is_in_multi_request')),
                                          array(new ComparedTable('partner_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_is_in_multi_request, 0)')));
		$this->compareAggregation(array(new ComparedTable('partner_id', 'borhandw.dwh_fact_api_calls', 'is_admin')),
                                          array(new ComparedTable('partner_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_is_admin, 0)')));
		$this->compareAggregation(array(new ComparedTable('action_id', 'borhandw.dwh_fact_api_calls', 'success')),
                                          array(new ComparedTable('action_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_success, 0)')));
                $this->compareAggregation(array(new ComparedTable('action_id', 'borhandw.dwh_fact_api_calls', '*')),
                                          array(new ComparedTable('action_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_calls, 0)')));
                $this->compareAggregation(array(new ComparedTable('action_id', 'borhandw.dwh_fact_api_calls', 'duration_msecs')),
                                          array(new ComparedTable('action_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(sum_duration_msecs, 0)')));
                $this->compareAggregation(array(new ComparedTable('action_id', 'borhandw.dwh_fact_api_calls', 'is_in_multi_request')),
                                          array(new ComparedTable('action_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_is_in_multi_request, 0)')));
                $this->compareAggregation(array(new ComparedTable('action_id', 'borhandw.dwh_fact_api_calls', 'is_admin')),
                                          array(new ComparedTable('action_id', 'borhandw.dwh_hourly_api_calls', 'ifnull(count_is_admin, 0)')));

		$this->compareAggregation(array(new ComparedTable('error_code_id', 'borhandw.dwh_fact_errors', '*')),
                                          array(new ComparedTable('error_code_id', 'borhandw.dwh_hourly_errors', 'ifnull(count_errors, 0)')));
	}	
	...
	private function AssertEntity($filename, $entityName, $fileID, $tableEntityName)
	{
		$collection = call_user_func($countPerEntityCallBack, $filename);
                $this->assertEquals(count($collection), DWHInspector::countDistinct('borhandw_ds.ds_api_', $fileID, $tableEntityName), $countPerEntityCallBack[1]);
