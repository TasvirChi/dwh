...
{
	private static Class<?> PKG = MySQLTableOutputMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!
																// $NON-NLS-1$
		...
		if (super.init(smi, sdi))
		{
			try
			{
				...
				if (data.batchMode && data.specialErrorHandling)
				{
					...
					if (log.isBasic())
						logBasic(BaseMessages.getString(PKG, "TableOutput.Log.BatchModeDisabled"));
				}

				if (meta.getDatabaseMeta() == null)
				{
					throw new KettleException(BaseMessages.getString(PKG,
							"TableOutput.Exception.DatabaseNeedsToBeSelected"));
				}
				if (meta.getDatabaseMeta() == null)
				{
